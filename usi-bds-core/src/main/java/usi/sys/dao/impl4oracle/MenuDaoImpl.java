...
public class MenuDaoImpl extends JdbcDaoSupport4oracle implements MenuDao {
	...
	public void insertMenu(final Menu menu){
		...
		int menuId = 0;
		final String sql = "insert into sys_menu(menu_id,menu_name,menu_action,parent_menu_id,display_order,"
				+ "is_leaf,menu_memo,status ) "
				+ " values(sys_menu_seq.nextval,?,?,?,?,?,?,?) ";
	...
	public int deleteMenuRole(Long menuId  ){
		String sql = "delete from sys_role_menu_rel t where t.menu_id = ?";
		return this.getJdbcTemplate().update(sql, new Object[]{menuId} );
	}
	/**
	 * 更新菜单表中的数据
