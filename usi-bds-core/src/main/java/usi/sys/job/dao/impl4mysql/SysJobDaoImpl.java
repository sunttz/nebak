...
@MysqlDb
@Repository
public class SysJobDaoImpl extends JdbcDaoSupport4mysql implements SysJobDao {

	@Override
	public void insertJob(final SysJob job) {
		String sql = " insert into sys_job (job_name, job_group,"
				+ "   job_bean_name, cron_expr, curr_state, "
				+ "   is_auto_start, ip_addr, is_del, "
				+ "   job_memo, staff_id, in_param, "
				+ "   create_time) "
				+ " values (?, ?, ?, ?, 0, ?, ?, 0, ?, ?, ?, now()) ";
	...
	public void updateJob(final SysJob job) {
		String sql = " update sys_job set job_name = ?,t.job_group = ?, "
				+ " job_bean_name = ?, cron_expr = ?, "
				+ " is_auto_start = ?, ip_addr = ?, "
				+ " job_memo = ?,  in_param = ?, "
				+ " last_mod_time = now() where t.job_id=? ";
		this.getJdbcTemplate().update(sql, new PreparedStatementSetter() {
	...
	public void updateCurrentState(long jobId, int currState) {
		String sql = " update sys_job set curr_state=? ,last_mod_time=now() where job_id=? ";
	...
	public void deleteJob(long jobId) {
		String sql = " update sys_job set is_del=1, last_mod_time=now() where job_id=? ";
	...
	public List<SysJob> getAutoStartJob() {
		String sql = " select t.job_id, t.job_name, t.job_group,"
				+ " t.job_bean_name, t.cron_expr, "
				+ " t.curr_state, t.is_auto_start, "
				+ " t.ip_addr, "
				+ " date_format(t.create_time,'%y-%m-%d %h:%i:%s'),"
				+ " in_param "
				+ " from sys_job t "
				+ " where t.is_del = 0 and t.is_auto_start=1 ";
